...
import { startAddPost, startSavePost } from '@/store/post/thunks';
...
const FormDialog = (props: FormDialogProps) => {
  ...
  const {
    ...
    defaultValues: props.postToUpdate,
  });
  ...
  useEffect(() => {
    if (messageSaved !== null && messageSaved.length > 0) {
  ...
  const onSavePost = async (postToSave: FormData) => {    
    try {
      if (!postToSave?.id) {
        ...
        dispatch(startAddPost(postToCreate));
      } else {
        dispatch(startSavePost(postToSave));
      }
